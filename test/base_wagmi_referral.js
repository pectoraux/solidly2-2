...
describe("core", function () {
  ...
  it("deploy BaseV1Voter", async function () {
    const BaseV1GaugeFactory = await ethers.getContractFactory("BaseV1GaugeFactory2");
    gauges_factory = await BaseV1GaugeFactory.deploy();
    await gauges_factory.deployed();
    const BaseV1Voter = await ethers.getContractFactory("WagmiReferralVoter");
    gauge_factory = await BaseV1Voter.deploy(ve.address, gauges_factory.address);
  ...
  it("deploy BaseV1Minter", async function () {
    ...
    await gauge_factory.setMinter(minter.address);
  });
  ...
  it("deploy BaseV1Factory gauge", async function () {
    ...
    await gauge_factory.createGauge(pair.address, 1);
    await gauge_factory.createGauge(pair2.address, 1);
    await gauge_factory.createGauge(pair3.address, 1);
    ...
    const Gauge = await ethers.getContractFactory("Gauge2");
  ...
  it("add gauge rewards", async function () {
    const pair_1000 = ethers.BigNumber.from("1000000000");
  ...
  it("gauge vote balanceOf", async function () {
  ...
  //   await minter.initialize([owner.address],[ethers.BigNumber.from("1000000000000000000")], ethers.BigNumber.from("1000000000000000000"));
    ...
    await network.provider.send("evm_mine")
    console.log("staking ==>", await staking.rewardRate());
    console.log("staking ==>", await gauge.rewardRate(ve_underlying.address));
    // expect(await gauge.rewardRate(ve_underlying.address)).to.be.equal(await staking.rewardRate());
  });
  ...
  it("vote for gauges", async function () {
    await gauge_factory.reset(1);
    await gauge_factory.reset(4);
    await gauge_factory.distro();
    console.log("claimable ==>", await gauge_factory.claimable(gauge.address));
    console.log("claimable ==>", await gauge_factory.claimable(gauge2.address));
    console.log("supplied ==>", await gauge_factory.weights(pair.address));
    console.log("supplied ==>", await gauge_factory.weights(pair2.address));
    ...
    await gauge_factory.vote(4, [pair.address], [1]);
    await gauge_factory.vote(4, [pair.address], [1]);
    expect(await gauge_factory.weights(pair.address)).to.be.below((await ve.balanceOfNFT(4)).mul(2));
    await network.provider.send("evm_increaseTime", [86400 * 7])
    await network.provider.send("evm_mine")
    await gauge_factory.vote(4, [pair.address], [1]);
    expect(await gauge_factory.weights(pair.address)).to.be.above((await ve.balanceOfNFT(4)).mul(2));
    await gauge_factory.vote(5, [pair2.address], [1]);
    const pair_1000 = ethers.BigNumber.from("1000000000");
    await ve_underlying.approve(gauge_factory.address, pair_1000);
    await gauge_factory.notifyRewardAmount(pair_1000);
    await gauge_factory.updateAll();
    ...
    console.log("supplied ==>", await gauge_factory.weights(pair.address));
    console.log("supplied ==>", await gauge_factory.weights(pair2.address));
    console.log(await gauge_factory.listing_fee());
    // expect(await gauge_factory.usedWeights(1)).to.equal(await gauge_factory.votes(1, pair.address));
  });
