...
describe("core", function () {
  ...
  it("3) create ask order", async function () {
    await minterFactory.createGauge("uber", "usd", "usd")
  ...
  it("4) buy with contract", async function () {
    ...
    await nftSvg.updateDescription(nfticketHelper2.address, "This is a receipt of your purchase of a collectible in the eCollectible marketplace. This is a receipt of your purchase of a collectible in the eCollectible marketplace. This is a receipt of your purchase of a collectible in the eCollectible marketplace. This is a receipt of your purchase of a collectible in the eCollectible marketplace. This is a receipt of your purchase of a collectible in the eCollectible marketplace.")

    console.log("nfticketHelper2.ownerOf(1)===============>", await nfticketHelper2.ownerOf(1))
    console.log("nfticketHelper2.tokenURI===============>", await nfticketHelper2.tokenURI(1))
  }).timeout(10000000);
  ...
  it("17) Buy discounted with account limits", async function () {
    ...
    console.log("nfticket 6==================>", await nfticket.getTicketInfo(6))
  ...
  it("30) Effects of Buy - vote/nfticket/lottery/treasury/cashbackfund/recurring revenue ", async function () {
    ...
    expect(await marketPlaceTrades.pendingRevenue(ust.address,1)).to.equal(110) //63 + 8 + 39 (netPrice + referrerFee) //#######
  ...
  it("33) Burn NFT for credit", async function () {
    ...
    console.log("nfticket==============>", await nfticket.getTicketInfo(9))
  ...
  it("35) claimPendingRevenue", async function () {
    ...
    expect(await marketPlaceTrades.pendingRevenue(ust.address,1)).to.equal(110)
    ...
    expect(await await ust.balanceOf(owner.address)).to.equal(ustBalanceBefore.add(110))
  ...
  it("39) Buy through incrementing auctions", async function () {
    expect((await nfticketHelper2.balanceOf(owner2.address))).to.equal(3);
    // create and auction
    await minterFactory.createGauge("Chimpanzee", "usd", "usd")
    await marketPlaceOrders.createAskOrder(
      "Chimpanzee",
      10,
      100000,
  ...
  it("40) Buy through decrementing auctions", async function () {
    // create and auction
    await minterFactory.createGauge("Chimpanzee2", "usd", "usd")
    await marketPlaceOrders.createAskOrder(
      "Chimpanzee2",
      10,
      100000,
  ...
  it("41) Ask order with drop-in timer", async function () {
    // create and auction
    await minterFactory.createGauge("Chimpanzee3", "usd", "usd")
    await marketPlaceOrders.createAskOrder(
      "Chimpanzee3",
      10,
      0,
    ...
    DefaultMinter = await ethers.getContractFactory('Minter')

    const defaultMinter = DefaultMinter.attach((await marketPlaceHelper3.minter(1,"Chimpanzee3")).minter)
    expect((await defaultMinter.balanceOf(owner.address))).to.equal(0);
    expect((await nfticketHelper2.balanceOf(owner.address))).to.equal(8);
    await marketPlaceTrades.buyWithContract(
      owner.address,
      owner.address,
    ...
    expect((await defaultMinter.balanceOf(owner.address))).to.equal(1);
    expect((await nfticketHelper2.balanceOf(owner.address))).to.equal(9);
    await defaultMinter.updateMaxSupply(10);

  }).timeout(10000000);

  it("42) AskOrder for custom minter", async function () {
    CustomMinter = await ethers.getContractFactory('CustomMinter')
    
    customMinter = await CustomMinter.deploy("usd", "usd")
    ...
    await marketPlaceOrders.createAskOrder(
      ...
      true,
      [0,10,0,0],
      customMinter.address,
      ust.address,
      ve.address
    );

    expect((await customMinter.balanceOf(owner.address))).to.equal(0);
    expect((await nfticketHelper2.balanceOf(owner.address))).to.equal(9);
    await ust.approve(marketPlaceTrades.address, 10)
    await marketPlaceTrades.buyWithContract(
      owner.address,
    ...
    console.log("mintvalues==============>", await marketPlaceHelper3.minter(1,"blablacar"))
    expect((await marketPlaceHelper3.minter(1,"blablacar")).minter).to.equal(customMinter.address)
    expect((await marketPlaceHelper3.minter(1,"blablacar")).tokenId).to.equal(0)
    expect((await marketPlaceHelper3.minter(1,"blablacar")).nftype).to.equal(1)
    expect((await nfticketHelper2.balanceOf(owner.address))).to.equal(10);
    expect((await customMinter.balanceOf(owner.address))).to.equal(1);
      
  })

  it("43) AskOrder for external erc721 token", async function () {
    await customMinter.setApprovalForAll(marketPlaceHelper3.address, true)
    await marketPlaceOrders.createAskOrder(
      "airbnb",
      10,
      0,
    ...
    console.log("mintvalues==============>", await marketPlaceHelper3.minter(1,"airbnb"))
    expect((await marketPlaceHelper3.minter(1,"airbnb")).minter).to.equal(customMinter.address)
    expect((await marketPlaceHelper3.minter(1,"airbnb")).tokenId).to.equal(1)
    expect((await marketPlaceHelper3.minter(1,"airbnb")).nftype).to.equal(1)
    expect((await nfticketHelper2.balanceOf(owner.address))).to.equal(11);
    expect((await customMinter.balanceOf(owner.address))).to.equal(1);

  })

  it("44) AskOrder for external erc1155 token", async function () {
    ERC1155_ = await ethers.getContractFactory('ERC1155_')
    console.log("mintvalues======1")
    token1155 = await ERC1155_.deploy("usd")
    await token1155.deployed()
    console.log("mintvalues======2")

    await token1155.mint(owner.address,1,1)
    console.log("mintvalues======3")

    await token1155.setApprovalForAll(marketPlaceHelper3.address, true)
    console.log("mintvalues======4")
    await marketPlaceOrders.createAskOrder(
      "payswap",
      10,
      0,
    ...
    console.log("mintvalues======5")
    
    expect((await token1155.balanceOf(owner.address,1))).to.equal(0);
    console.log("mintvalues======7")
    expect((await nfticketHelper2.balanceOf(owner.address))).to.equal(11);
    await ust.approve(marketPlaceTrades.address, 10)
    console.log("mintvalues======8")
    await marketPlaceTrades.buyWithContract(
      owner.address,
      owner.address,
      owner.address,
    ...
    console.log("mintvalues==============>", await marketPlaceHelper3.minter(1,"payswap"))
    expect((await marketPlaceHelper3.minter(1,"payswap")).minter).to.equal(token1155.address)
    expect((await marketPlaceHelper3.minter(1,"payswap")).tokenId).to.equal(1)
    expect((await marketPlaceHelper3.minter(1,"payswap")).nftype).to.equal(2)
    expect((await nfticketHelper2.balanceOf(owner.address))).to.equal(12);
    expect((await token1155.balanceOf(owner.address,1))).to.equal(1);

  })

  // it("45) cancelAskOrder", async function () {
  ...
  // it("46) closeCollectionForTradingAndListing", async function () {
  //   expect(await marketPlaceCollection.addressToCollectionId(owner.address)).to.equal(1)
