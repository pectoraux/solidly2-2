...
describe("core", function () {
  ...
  await card.setContractAddress(contractAddresses.address)
  ...
  await card.updateVe(ve.address)
}).timeout(10000000);
  ...
  it("5) create card", async function () {
    await card.connect(owner2).updateTokenId(1)
    await card.connect(owner3).updateTokenId(2)

    console.log("balance before============>", await card.balance(owner2.address, ust.address), await card.balance(owner3.address, ust.address))
    await ust.connect(owner2).approve(card.address, 10)
    await card.connect(owner2).addBalance(owner2.address, ust.address, 10)

    await ust.connect(owner3).approve(card.address, 10)
    await card.connect(owner3).addBalance(owner3.address, ust.address, 10)
    console.log("balance after============>", await card.balance(owner2.address, ust.address), await card.balance(owner3.address, ust.address))
