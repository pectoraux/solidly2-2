...
describe("core", function () {
  ...
  let Paywall;
  let paywallARP;
  let paywallARPHelper;
  let paywallARPFactory;
  let nftSvg;
  ...
  it("1) deploy market place", async function () {
    ...
    const MarketPlaceOrders = await ethers.getContractFactory("contracts/PaywallMarketPlace.sol:PaywallMarketPlaceOrders");
    const MarketPlaceOrders02 = await ethers.getContractFactory("contracts/MarketPlace.sol:MarketPlaceOrders");
    const MarketPlaceTrades = await ethers.getContractFactory("contracts/PaywallMarketPlace.sol:PaywallMarketPlaceTrades");
    const MarketPlaceHelper = await ethers.getContractFactory("contracts/PaywallMarketPlace.sol:PaywallMarketPlaceHelper");
    const MarketPlaceHelper3 = await ethers.getContractFactory("contracts/PaywallMarketPlace.sol:PaywallMarketPlaceHelper3");
  ...
  Paywall = await ethers.getContractFactory("contracts/PaywallMarketPlace.sol:Paywall");
  PaywallARPHelper = await ethers.getContractFactory("contracts/PaywallMarketPlace.sol:PaywallARPHelper");
  PaywallARPFactory = await ethers.getContractFactory("contracts/PaywallMarketPlace.sol:PaywallARPFactory");
  ...
  const MarketPlaceHelper2 = await ethers.getContractFactory("contracts/PaywallMarketPlace.sol:PaywallMarketPlaceHelper2",{
    libraries: {
      Percentile: percentile.address,
    },
  });
  ...
  paywallARPFactory = await PaywallARPFactory.deploy();
  await paywallARPFactory.deployed()

  paywallARPHelper = await PaywallARPHelper.deploy();
  ...
  await contractAddresses.setPaywallMarketHelpers3(marketPlaceHelper3.address)
  ...
  await contractAddresses.setPaywallMarketHelpers2(marketPlaceHelper2.address)
  ...
  await contractAddresses.setPaywallMarketHelpers(marketPlaceHelper.address)
  console.log("contractAddresses.setPaywallMarketHelpers===========> Done!")
  
  await contractAddresses.setPaywallMarketTrades(marketPlaceTrades.address)
  ...
  await contractAddresses.setPaywallMarketOrders(marketPlaceOrders.address)
  ...
  it("17) Buy discounted with account limits", async function () {
    ...
    const cursor = (await nfticket.getTicketInfo(1)).date
  ...
  it("23) Receive cashback in cash", async function () {
    ...
    const cursor = (await nfticket.getTicketInfo(6)).date
    await marketPlaceOrders.modifyAskOrderCashbackPriceReductors(
      "uber",
      1,   
      0,   
  ...
  it("24) Register user with identity limits", async function () {
    ...
    await marketPlaceHelper03.emitUserRegistration(1, 1, 3, 1, true)
  }).timeout(10000000);
  ...
  it("25) Add referral with identity limits", async function () {
    ...
    // revert since user is not yet partner
    await expect(marketPlaceOrders.addReferral(
      owner.address,
      owner.address,
      "uber",
    ...
    // partner
    expect(await marketPlaceHelper02.partnerShip(1,1)).to.equal(false)
    await marketPlaceOrders02.addReferral(
      owner.address,
      owner.address,
      "uber",
      [1000,0,3]
    )
    expect(await marketPlaceHelper02.partnerShip(1,1)).to.equal(true)
    // revert since owner has not set canPublish
    await expect(marketPlaceOrders.addReferral(
      owner.address,
      owner.address,
      "uber",
    ...
    expect(await marketPlaceHelper2.getCanPublish(1,"uber")).to.equal(false)
    await marketPlaceHelper2.updateCanPublish("uber", true)
    expect(await marketPlaceHelper2.getCanPublish(1,"uber")).to.equal(true)
  ...
  it("29) create gauge", async function () {
    ...
    // await businessBribe.setContractAddress(contractAddresses.address)
  ...
  it("30) Effects of Buy - vote/nfticket/lottery/treasury/cashbackfund/recurring revenue ", async function () {
    ...
    expect(await marketPlaceTrades.pendingRevenue(ust.address,1)).to.equal(63)
    ...
    // await referralBribe.setContractAddress(contractAddresses.address)
    ...
    expect(await referralVoter.pools(ve.address,0)).to.equal(1)
    expect(await nfticketHelper.lotteryCredits(owner.address, ve.address)).to.equal(0)
    ...
    console.log("lotteryCredits2==============>", await nfticketHelper.lotteryCredits(owner2.address, ust.address))
    ...
    expect(await marketPlaceTrades.pendingRevenue(ust.address,1)).to.equal(119) //63 + 8 + 48 (netPrice + referrerFee) //#######
  ...
  it("31) Mint external nfticket", async function () {
    ...
    console.log("external tickets================>", await nfticket.getTicketInfo(9))
  }).timeout(10000000);
  ...
  it("34) Sponsor tag", async function () {
    ...
    expect((await nfticketHelper.scheduledMedia(1)).message).to.equal("https://link-to-media.com")
  ...
  it("35) claimPendingRevenue", async function () {
    ...
    expect(await marketPlaceTrades.pendingRevenue(ust.address,1)).to.equal(119)
    ...
    expect(await await ust.balanceOf(owner.address)).to.equal(ustBalanceBefore.add(119))
  ...
  it("37) Transfer future revenue to note", async function () {
    ...
    expect(await marketPlaceTrades.pendingRevenueFromNote(ust.address,2)).to.equal(60) //##########
    ...
    expect(await marketPlaceTrades.cashbackFund(ust.address,1)).to.equal(cashBackBefore.add(0)); //##########
  ...
  it("38) Claim Pending from note", async function () {
    ...
    expect(await marketPlaceTrades.pendingRevenueFromNote(ust.address,2)).to.equal(0)
    expect(await await ust.balanceOf(owner.address)).to.equal(userBalanceBefore.add(60)) //##########
  ...
  it("42) subscription renewal", async function () {
    await paywallARPFactory.createGauge();
    ...
    expect(paywallAddress).to.equal(paywallARP.address)

    await paywallARP.updateProtocol(6, 0, [owner.address])

    console.log("4===============>", await nfticket.getTicketInfo(6))
    console.log("options===============>", await nfticketHelper2.getTicketPaywallOptions(6))
    // console.log("================>", await marketPlaceHelper.paywallOptions(1, "uber", 0))

    console.log("next due before time increase=====================>", await paywallARPHelper.getDueReceivable(paywallARP.address, 1))
    expect(await paywallARP.ongoingSubscription(owner.address, 6, "uber")).to.equal(true)
    expect((await paywallARPHelper.getDueReceivable(paywallARP.address, 1))[0]).to.equal(0)
    expect((await paywallARPHelper.getDueReceivable(paywallARP.address, 1))[2]).to.equal(0)

    expect(await paywallARP.getState(owner.address, "uber", 101)).to.equal(101)
    await paywallARP.updateDiscountDivisor(0,100,1000,100)
    console.log("getDueReceivable=====================>", await paywallARPHelper.getDueReceivable(paywallARP.address, 1), await paywallARP.owner(1), owner.address)
    expect(await paywallARP.getState(owner.address, "uber", 101)).to.equal(101)
    await expect(paywallARP.autoCharge(1, 3)).to.be.reverted;
    expect((await paywallARP.protocolInfo(1)).paidReceivable).to.equal(0)
    expect((await paywallARPHelper.getDueReceivable(paywallARP.address, 1))[0]).to.equal(0)

    // increase time
    await network.provider.send("evm_increaseTime", [2592000 + 3000])
    await network.provider.send("evm_mine")

    console.log("protocolInfo before===========>", await paywallARP.protocolInfo(1), await ust.balanceOf(owner.address))
    await ust.approve(marketPlaceTrades.address, 101);
    await paywallARP.autoCharge(1, 3)
    console.log("protocolInfo after===========>", await paywallARP.protocolInfo(1))
    expect((await paywallARP.protocolInfo(1)).paidReceivable).to.equal(12)
    console.log("next due after time increase=====================>", await paywallARPHelper.getNumPeriods(1688947074, 0, 2592000), await paywallARPHelper.getDueReceivable(paywallARP.address, 1))
    // expect((await paywallARPHelper.getDueReceivable(paywallARP.address, 1))[0]).to.equal(12)
    expect((await paywallARPHelper.getDueReceivable(paywallARP.address, 1))[2]).to.be.below(3010)
    expect(await paywallARP.ongoingSubscription(owner.address, 6, "uber")).to.equal(true)

    expect(await paywallARP.getState(owner.address, "uber", 101)).to.equal(101)
    await paywallARP.updatePenaltyDivisor(0, 100,1000,0)
    // expect(await paywallARP.getState(owner.address, "uber", 101)).to.equal(104)

    expect((await paywallARP.protocolInfo(1)).paidReceivable).to.equal(12)
    expect(await paywallARPHelper.isGauge(paywallARP.address)).to.equal(true)
    expect(await paywallARP.owner(1)).to.equal(owner.address)
    
    // await ust.approve(marketPlaceTrades.address, 104);
    // await paywallARP.autoCharge(1, 3)
    // expect((await paywallARP.protocolInfo(1)).paidReceivable).to.equal(24)
    
    // console.log("getDueReceivable=====================>", await paywallARPHelper.getDueReceivable(paywallARP.address, 1))
    // expect(await paywallARP.ongoingSubscription(owner.address, 6, "uber")).to.equal(true)

    expect((await paywallARP.protocolInfo(1)).autoCharge).to.equal(false)
    await paywallARP.updateAutoCharge(1, true)
    expect((await paywallARP.protocolInfo(1)).autoCharge).to.equal(true)
    
    console.log("partner before==========>", await paywallARP.partnershipEnds(1,"uber"))
    await paywallARP.partner(1,"uber","uber",2592000, false)
    console.log("partner after==========>", await paywallARP.partnershipEnds(1,"uber"))
  })

  it("43) cancelAskOrder", async function () {
  ...
  it("44) closeCollectionForTradingAndListing", async function () {
    expect(await marketPlaceCollection.addressToCollectionId(owner.address)).to.equal(1)
