...
describe("core", function () {
  ...
  it("3) create gauge", async function () {
    ...
    await contributorVoter.createGauge(ve.address)
    gauge = await contributorVoter.gauges(1,ve.address)
    
    console.log("gauge==================>", gauge)
    console.log("bribe==================>", await contributorVoter.bribes(gauge))
    console.log("poolForGauge==================>", await contributorVoter.poolForGauge(gauge))
    console.log("isGauge==================>", await contributorVoter.isGauge(gauge))
    console.log("pools==================>", await contributorVoter.pools(ve.address,0))

    referralBribe = ReferralBribe.attach(await contributorVoter.bribes(gauge))
    businessGauge = BusinessGauge.attach(gauge)

    expect(gauge).to.not.equal("0x0000000000000000000000000000000000000000")
    expect(await contributorVoter.bribes(gauge)).to.not.equal("0x0000000000000000000000000000000000000000")
    expect(await contributorVoter.poolForGauge(gauge)).to.not.equal("0x0000000000000000000000000000000000000000")
    expect(await contributorVoter.isGauge(gauge)).to.equal(true)
    expect((await contributorVoter.pools(ve.address,0))).to.equal(1)
  });

  it("4) vote", async function () {
    expect(await referralBribe.balanceOf(1)).to.equal(0)
    expect(await contributorVoter.weights(1,ve.address)).to.equal(0)

    await contributorVoter.vote(1,1,businessGauge.address, ve.address, true)
    
    console.log("weights============>", await contributorVoter.weights(1,ve.address))
    console.log("balanceOf============>", await referralBribe.balanceOf(1))
    expect(await contributorVoter.weights(1,ve.address)).to.not.equal(0)
    expect(await referralBribe.balanceOf(1)).to.not.equal(0)

  });

  it("5) distribute", async function () {
    console.log("voter balance before============>", await ve_underlying.balanceOf(contributorVoter.address))
    console.log("gauge balance before============>", await ve_underlying.balanceOf(businessGauge.address))
    expect(await ve_underlying.balanceOf(contributorVoter.address)).to.equal(0)
    ...
    await contributorVoter.distribute(businessGauge.address, ve.address)
    ...
    expect(await ve_underlying.balanceOf(contributorVoter.address)).to.not.equal(0)
