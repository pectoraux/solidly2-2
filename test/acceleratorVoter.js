...
describe("core", function () {
  ...
  it("3) create gauge", async function () {
    ...
    await acceleratorVoter.createGauge(ve.address)
    gauge = await acceleratorVoter.gauges(1,ve.address)
    
    console.log("gauge==================>", gauge)
    console.log("bribe==================>", await acceleratorVoter.bribes(gauge))
    console.log("poolForGauge==================>", await acceleratorVoter.poolForGauge(gauge))
    console.log("isGauge==================>", await acceleratorVoter.isGauge(gauge))
    console.log("pools==================>", await acceleratorVoter.pools(ve.address,0))

    referralBribe = ReferralBribe.attach(await acceleratorVoter.bribes(gauge))
    businessGauge = BusinessGauge.attach(gauge)

    expect(gauge).to.not.equal("0x0000000000000000000000000000000000000000")
    expect(await acceleratorVoter.bribes(gauge)).to.not.equal("0x0000000000000000000000000000000000000000")
    expect(await acceleratorVoter.poolForGauge(gauge)).to.not.equal("0x0000000000000000000000000000000000000000")
    expect(await acceleratorVoter.isGauge(gauge)).to.equal(true)
    expect((await acceleratorVoter.pools(ve.address,0))).to.equal(1)
  });
  ...
  it("4) vote", async function () {
    expect(await referralBribe.balanceOf(1)).to.equal(0)
    expect(await acceleratorVoter.weights(1,ve.address)).to.equal(0)

    await acceleratorVoter.vote(1,1,businessGauge.address, ve.address, true)
    
    console.log("weights============>", await acceleratorVoter.weights(1,ve.address))
    console.log("balanceOf============>", await referralBribe.balanceOf(1))
    expect(await acceleratorVoter.weights(1,ve.address)).to.not.equal(0)
    expect(await referralBribe.balanceOf(1)).to.not.equal(0)

  });

  it("5) distribute", async function () {
    console.log("voter balance before============>", await ve_underlying.balanceOf(acceleratorVoter.address))
    console.log("gauge balance before============>", await ve_underlying.balanceOf(businessGauge.address))
    expect(await ve_underlying.balanceOf(acceleratorVoter.address)).to.equal(0)
    ...
    await acceleratorVoter.distribute(businessGauge.address, ve.address)

    console.log("currentVolume==========>", await businessMinter.currentVolume(ve.address))
    console.log("weekly emission==========>", await businessMinter.weekly_emission(ve.address))
    expect(await ve_underlying.balanceOf(acceleratorVoter.address)).to.not.equal(0)
    expect(await ve_underlying.balanceOf(businessGauge.address)).to.not.equal(0)
    console.log("acceleratorVoter balance after============>", await ve_underlying.balanceOf(acceleratorVoter.address))
    console.log("contributorVoter balance after============>", await ve_underlying.balanceOf(contributorVoter.address))
    console.log("businessVoter balance after============>", await ve_underlying.balanceOf(businessVoter.address))
    console.log("referralVoter balance after============>", await ve_underlying.balanceOf(referralVoter.address))
