...
describe("core", function () {
  ...
  it("deploy valentine NFT", async function () {
    [owner, owner2, owner3] = await ethers.getSigners(3);
    token = await ethers.getContractFactory("Token");
    ValentineNFT = await ethers.getContractFactory("ValentineNFT");
    ...
    await valentineNFT.deployed();

  });

  // it("mint male to male", async function () {
  //   console.log("1==>", await valentineNFT.getMaleSetLength())
  //   // owner 2
  //   expect(await valentineNFT.totalSupply_()).to.equal(0);
  //   await ust.connect(owner2).approve(valentineNFT.address, 20);
  //   await valentineNFT.connect(owner2).batchMint(owner2.address, 0, 20, 1, 1);
  //   console.log("pending round", await valentineNFT.pendingRound(1));
  //   expect(await valentineNFT.totalSupply_()).to.equal(0);
  //   console.log("2==>", await valentineNFT.getMaleSetLength())
  //   // owner 3
  //   await ust.connect(owner3).approve(valentineNFT.address, 20);
  //   await valentineNFT.connect(owner3).batchMint(owner3.address, 0, 20, 1, 1);
  //   console.log("pending round", await valentineNFT.pendingRound(1));

  //   console.log("3==>", await valentineNFT.getMaleSetLength())
  //   console.log(await valentineNFT.totalSupply_())
  //   console.log("ticket 1", await valentineNFT.ticketInfo_(1));
  //   console.log("ticket 2", await valentineNFT.ticketInfo_(2));
  // });

  it("mint male to female", async function () {
    console.log("1==>", await valentineNFT.getMaleSetLength())
    // owner 2
    expect(await valentineNFT.totalSupply_()).to.equal(0);
    await ust.connect(owner2).approve(valentineNFT.address, 20);
    await valentineNFT.connect(owner2).batchMint(owner2.address, 0, 20, 1, 2);
    console.log("pending round", await valentineNFT.pendingRound(1));
    expect(await valentineNFT.totalSupply_()).to.equal(0);
    console.log("2==>", await valentineNFT.getMaleSetLength())
    // owner 3
    await ust.connect(owner3).approve(valentineNFT.address, 50);
    await valentineNFT.connect(owner3).batchMint(owner3.address, 0, 50, 2, 1);
    console.log("pending round", await valentineNFT.pendingRound(1));
  ...
  it("send message", async function () {
    await valentineNFT.connect(owner2).messageValentine(2, "Hey luv");
    await valentineNFT.connect(owner3).messageValentine(1, "Hey sugar");
  ...
  it("send token", async function () {
    const ust_before2 = await ust.balanceOf(owner2.address)
    const ve_before2 = await ve_underlying.balanceOf(owner2.address)
    const ust_before3 = await ust.balanceOf(owner3.address)
    const ve_before3 = await ve_underlying.balanceOf(owner3.address)

    await ust.connect(owner2).approve(valentineNFT.address, 100)
    await ve_underlying.connect(owner2).approve(valentineNFT.address, 100)
    await valentineNFT.connect(owner2).transferToValentine(ust.address, 2, 100);
    await valentineNFT.connect(owner2).transferToValentine(ve_underlying.address, 2, 100);

    expect(ust_before2.sub(await ust.balanceOf(owner2.address))).to.equal(100)
    expect((await ust.balanceOf(owner3.address)).sub(ust_before3)).to.equal(100)
    expect(ve_before2.sub(await ve_underlying.balanceOf(owner2.address))).to.equal(100)
    expect((await ve_underlying.balanceOf(owner3.address)).sub(ve_before3)).to.equal(100)
