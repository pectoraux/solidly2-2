...
contract AcceleratorBribe {
    ...
    */
    ...
    function getRewardForOwner(uint tokenId, address[] memory tokens) external lock  {
        require(msg.sender == IContract(contractAddress).acceleratorVoter(), "Only accelerator voter");
...
contract AcceleratorBribeFactory {
    ...
    function createBribe(address _ve) external returns (address) {
        last_gauge = address(new AcceleratorBribe(_ve));
        return last_gauge;
    }
}